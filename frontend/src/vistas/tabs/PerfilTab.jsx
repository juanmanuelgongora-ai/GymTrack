...
export default function PerfilTab({ token, userData }) {
      ...
      <div className="perfil-header p-24">
        ...
        <div className="perfil-info">
          ...
          <div className="perfil-info-tags">
            ...
            <span className="perfil-tag">📍 {perfil?.user?.email || ''}</span>
            <span className="perfil-tag"><Shield size={14} className="mr-12" color="#4ade80" /> GymTrack</span>
          </div>
          <div className="perfil-stats">
            <div className="p-stat"><h3>{latestMetrica?.peso_kg || cliente?.peso_kg || '--'} <span className="text-secondary text-sm font-normal">kg</span></h3><p>Peso Actual</p></div>
            <div className="p-stat"><h3>{cliente?.altura_cm || latestMetrica?.altura_cm || '--'} <span className="text-secondary text-sm font-normal">cm</span></h3><p>Altura</p></div>
            <div className="p-stat"><h3>{latestMetrica?.imc || cliente?.imc || '--'}</h3><p>IMC</p></div>
          </div>
        </div>
      </div>
      ...
      {activeSection === 'personal' && (
        <div className="perfil-split">
          <div className="glass-panel p-24">
            ...
            <div className="data-row"><span className="data-label">Email</span><span className="data-value">{perfil?.user?.email || '--'}</span></div>
            <div className="data-row"><span className="data-label">Género</span><span className="data-value">{cliente?.genero === 'M' ? 'Masculino' : cliente?.genero === 'F' ? 'Femenino' : '--'}</span></div>
            ...
            <div className="data-row"><span className="data-label">Rol</span><span className="data-value" style={{ textTransform: 'capitalize' }}>{perfil?.user?.rol || '--'}</span></div>
          </div>
          <div className="glass-panel p-24">
            <h3 className="section-title mb-24 flex-align-center gap-12"><Activity size={20} color="#ff6b35" /> Datos Físicos y Objetivos</h3>
            <div className="data-row"><span className="data-label">Altura</span><span className="data-value">{cliente?.altura_cm || '--'} cm</span></div>
            <div className="data-row"><span className="data-label">Peso Actual</span><span className="data-value">{latestMetrica?.peso_kg || cliente?.peso_kg || '--'} kg</span></div>
            <div className="data-row"><span className="data-label">IMC</span><span className="data-value">{latestMetrica?.imc || cliente?.imc || '--'}</span></div>
            <div className="data-row"><span className="data-label">Objetivo Principal</span><span className="data-value">{cliente?.objetivo_principal || '--'}</span></div>
